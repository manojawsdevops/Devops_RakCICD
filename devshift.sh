...
then
    ...
    /usr/local/bin/oc new-app --image-stream=manojprod/devopsimage:v1 -e LICENSE=accept && /usr/local/bin/oc expose service devopsimage -l name=devopsimage --port=7800
fi
