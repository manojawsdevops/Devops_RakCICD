...
then
    ...
    /usr/local/bin/oc new-app --image-stream=manoj/devopsimage:v1 -e LICENSE=accept && /usr/local/bin/oc expose service devopsimage -l name=devopsimage oc create route passthrough --service devopsimage
--port=7843
fi
