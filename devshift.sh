...
then
    ...
    /usr/local/bin/oc new-app --image-stream=manoj/devopsimage:v1 -e LICENSE=accept && /usr/local/bin/oc create route passthrough devopsimage --service=devopsimage  --port=7843
fi
